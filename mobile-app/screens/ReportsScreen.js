...
export default function ReportsScreen() {
  ...
  const openAdd = () => { setEditing(null); setForm({ title: '', note: '' }); setVisible(true); };
  const openEdit = (item) => { setEditing(item); setForm({ title: item.title || '', note: item.note || '' }); setVisible(true); };
  const save = () => {
    if (!form.title.trim()) return;
    if (editing) updateReport({ ...editing, ...form }); else addReport(form);
    setVisible(false);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <WebOnlyAction title="Add Report" onPress={openAdd} style={styles.addBtn} textStyle={styles.addBtnText} />
      </View>
        ...
        <View style={styles.card}>
          ...
          {Platform.OS === 'web' && (
            <View style={styles.row}>
              ...
              <TouchableOpacity style={styles.deleteBtn} onPress={() => deleteReport(item.id)}><Text style={styles.deleteText}>Delete</Text></TouchableOpacity>
            </View>
          )}
        </View>
      )} />
      <Modal visible={visible} transparent animationType="slide">
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            <Text style={styles.modalTitle}>{editing ? 'Edit Report' : 'Add Report'}</Text>
            <TextInput style={styles.input} placeholder="Title" value={form.title} onChangeText={(t) => setForm((p) => ({ ...p, title: t }))} />
            <TextInput style={styles.input} placeholder="Note" value={form.note} onChangeText={(t) => setForm((p) => ({ ...p, note: t }))} />
            <View style={styles.row}>
