...
export default function UsersScreen() {
  ...
  const openAdd = () => { setEditing(null); setForm({ name: '', email: '' }); setVisible(true); };
  const openEdit = (item) => { setEditing(item); setForm({ name: item.name || '', email: item.email || '' }); setVisible(true); };
  const save = () => {
    if (!form.name.trim()) return;
    if (editing) updateUser({ ...editing, ...form }); else addUser(form);
    setVisible(false);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <WebOnlyAction title="Add User" onPress={openAdd} style={styles.addBtn} textStyle={styles.addBtnText} />
      </View>
        ...
        <View style={styles.card}>
          ...
          {Platform.OS === 'web' && (
            <View style={styles.row}>
              ...
              <TouchableOpacity style={styles.deleteBtn} onPress={() => deleteUser(item.id)}><Text style={styles.deleteText}>Delete</Text></TouchableOpacity>
            </View>
          )}
        </View>
      )} />
      <Modal visible={visible} transparent animationType="slide">
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            ...
            <TextInput style={styles.input} placeholder="Email" value={form.email} onChangeText={(t) => setForm((p) => ({ ...p, email: t }))} />
            <View style={styles.row}>
