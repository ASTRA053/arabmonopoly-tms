...
export default function VehiclesScreen() {
  ...
  const openAdd = () => { setEditing(null); setForm({ name: '', plate: '' }); setVisible(true); };
  const openEdit = (item) => { setEditing(item); setForm({ name: item.name || '', plate: item.plate || '' }); setVisible(true); };
  const save = () => {
    if (!form.name.trim()) return;
    if (editing) updateVehicle({ ...editing, ...form }); else addVehicle(form);
    setVisible(false);
  };
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <WebOnlyAction title="Add Vehicle" onPress={openAdd} style={styles.addBtn} textStyle={styles.addBtnText} />
      </View>
        ...
        <View style={styles.card}>
          ...
          {Platform.OS === 'web' && (
            <View style={styles.row}>
              ...
              <TouchableOpacity style={styles.deleteBtn} onPress={() => deleteVehicle(item.id)}><Text style={styles.deleteText}>Delete</Text></TouchableOpacity>
            </View>
          )}
        </View>
      )} />
      <Modal visible={visible} transparent animationType="slide">
        <View style={styles.modalBg}>
          <View style={styles.modalCard}>
            ...
            <TextInput style={styles.input} placeholder="Plate" value={form.plate} onChangeText={(t) => setForm((p) => ({ ...p, plate: t }))} />
            <View style={styles.row}>
